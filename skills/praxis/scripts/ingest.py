...
def parse_cv(cv_text):
    ...
    blocks = re.split(r'(DexCare:|Joint Interagency Task Force:|Lowbush Company:|Marriott Vacation Club:|AccessUSA \(Hotelbeds\):|Orlando\.com / Internet Vacations:|Body International:|Modis Technologies:|OCI:)', career_text)
    ...
    for i in range(1, len(blocks), 2):
        ...
        date_match = re.search(r'(\d{4}\s*-\s*(?:\d{4}|Present|\d{2}))', content, re.IGNORECASE)
        dates = date_match.group(1) if date_match else "Unknown"
        dates = re.sub(r'\s*-\s*', ' - ', dates)
        if dates == "2015 - 21": dates = "2015 - 2021"
        if dates == "2010 - 15": dates = "2010 - 2015"
        if dates == "2006 - 10": dates = "2006 - 2010"
        if dates == "2003 - 06": dates = "2003 - 2006"
        if dates == "2001 - 03": dates = "2001 - 2003"
        if dates == "2000 - 01": dates = "2000 - 2001"
        if dates == "1998 - 00": dates = "1998 - 2000"
        if dates == "1995 - 98": dates = "1995 - 1998"
        ...
        # Extract role
        role_match = re.search(r'Role:(.*?)(?:\Z|Accomplishments:)', content, re.DOTALL | re.IGNORECASE)
        if role_match:
...
def main():
    ...
    kb = {
        "UserProfile": {"name": "Kenton Smeltzer", "contact": {"phone": "786-933-0944", "email": "ksmeltzer@gmail.com", "linkedin": "http://www.linkedin.com/in/kentonsmeltzer", "github": "https://github.com/ksmeltzer"}},
        "CareerCatalog": [],
    ...
    for txt_file in glob.glob('*.txt'):
        if txt_file == 'skills.txt':
            with open('skills.txt', 'r') as f:
                skills = [s.strip() for s in f.readlines() if s.strip() and " at " not in s]
                kb["RelationalSkillsDatabase"] = {s: "Identified via input" for s in set(skills)}
            continue
        if txt_file == 'kenton_cv.txt': continue
    ...
    os.system('mv -f Kenton-Smeltzer-\ cv.pdf voya.txt skills.txt Basic_LinkedInDataExport_*.zip 083befb0-*.tar.gz kenton_cv.txt .praxis/sources/ 2>/dev/null || true')
