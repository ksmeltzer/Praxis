...
1.  **Scan & Extract (Deep Harvest Protocol)**: Search the root directory for *all* source files (`*.pdf`, `*.docx`, `*.txt`, `*.md`, `*.csv`, `*.zip`, `*.tar.gz`, `*.tgz`). **CRITICAL EXCLUSION**: You must EXPLICITLY IGNORE system files (`AGENTS.md`, `CLAUDE.md`, `README.md`) and hidden directories (like `.beads/`). Do NOT parse them as career history.
    ...
    *   **Date Standardization**: You MUST normalize all extracted dates into a consistent, properly spaced format (e.g., `YYYY - YYYY` or `MM/YYYY - Present`) before saving to the database to ensure the timeline remains uniform regardless of the source file's formatting.
...
4.  **Prompt & Deep Harvest**: Analyze what data is missing from the database.
    *   **LinkedIn Data**: Provide the direct link to the user: `https://www.linkedin.com/mypreferences/d/download-my-data`. Advise them that to get their full work history, they MUST request the **"Larger data archive"**. The smaller "Profile" bundle does NOT contain the `Positions.csv` file needed for work history. If `Positions.csv` is missing from an archive, you MUST NOT hallucinate or web-scrape to fill the gaps; simply process what is available or ask the user for the missing details directly.
...
4.  **The Expert Inquisitor & Contextual Researcher**: Analyze the *newly* merged data for weak points (missing metrics, vague responsibilities) and **Logical Timeline Anomalies**. 
    *   **Anomaly Detection**: Specifically check for overlapping roles, multiple jobs marked as "Present", or chronological errors (e.g., end dates preceding start dates). If multiple "Present" roles exist, immediately halt and prompt the user: *"I noticed you have multiple roles marked as 'Present'. Are you working these concurrently, or did one of these conclude?"*
    *   **Multi-Step Interview**: Conduct a sequential, multi-step interview wizard to extract exact numbers and clarify any flagged anomalies. **CRITICAL: Ask exactly ONE question at a time.** Wait for the user to answer before asking the next. Perform deep web searches on niche methodologies (e.g., CIA Animal Kingdom) to enrich the JSON database with authoritative context.
