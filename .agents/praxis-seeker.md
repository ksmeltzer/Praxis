...
- **Stale Links & 404s (GIVE UP QUICKLY)**: If the initial URL returns a 404, says "Job no longer available", "Position filled", or redirects to a generic company homepage instead of a job listing, the job is dead. DO NOT attempt to search the site to find it. Immediately abort, mark the job as `FAILED` in `applications.json` with `error_reason: "Job posting is stale, 404, or closed"`, and generate the diagnostic file.
